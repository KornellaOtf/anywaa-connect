...
import com.anywaa.connect.ads.ConsentManager
import com.anywaa.connect.embedding.RagServiceManager
import com.anywaa.connect.utils.FileUtils
import com.anywaa.connect.R
import java.io.File
import com.anywaa.connect.data.ThemeMode
import com.anywaa.connect.data.localFileName
...
fun SettingsScreen(
    ...
    val FeatureFlags.PREMIUM_ENABLED by (context.applicationContext as com.anywaa.connect.AnywaaConnectApplication)
        .billingManager.FeatureFlags.PREMIUM_ENABLED.collectAsState(initial = false)
        
    val themePreferences = remember { com.anywaa.connect.data.ThemePreferences(context) }
    val lowDataModeEnabled by themePreferences.lowDataModeEnabled.collectAsState(initial = false)
    val systemPrompt by themePreferences.systemPrompt.collectAsState(initial = "")
                ...
                SettingsSection(title = stringResource(R.string.features)) {
                    ...
                    Row(
                        ...
                        Icon(
                            ...
                            tint = if (FeatureFlags.PREMIUM_ENABLED) MaterialTheme.colorScheme.onSurfaceVariant
                        ...
                        Column(
                            ...
                            Row(verticalAlignment = Alignment.CenterVertically) {
                                Text(
                                    ...
                                    color = if (FeatureFlags.PREMIUM_ENABLED) MaterialTheme.colorScheme.onSurface
                            ...
                            Text(
                                text = if (!FeatureFlags.PREMIUM_ENABLED) stringResource(R.string.premium_tap_to_unlock)
                                ...
                                color = if (FeatureFlags.PREMIUM_ENABLED) MaterialTheme.colorScheme.onSurfaceVariant
                                        else MaterialTheme.colorScheme.onSurfaceVariant.copy(alpha = 0.5f)
                            )
                        }
                                ...
                                onCheckedChange = { enabled ->
                                    ...
                                    if (enabled) {
                                        ...
                                        coroutineScope.launch {
                                            try {
                                                val prefs = com.anywaa.connect.data.ThemePreferences(context)
                                                ...
                                                val db = com.anywaa.connect.data.AnywaaConnectDatabase.getDatabase(context)
                                                ...
                                                withContext(kotlinx.coroutines.Dispatchers.IO) {
                                                    val ragManager = com.anywaa.connect.embedding.RagServiceManager.getInstance(context)
                                                    try {
                                                        ...
                                                        val processor = com.anywaa.connect.data.MemoryProcessor(context, db)
                                                        processor.processPending()

                                                        try { withTimeoutOrNull(5_000) { com.anywaa.connect.data.MemoryProcessor.processing.first { it } } } catch (_: Exception) { }
                                                        com.anywaa.connect.data.MemoryProcessor.processing.first { running -> !running }
                                                    } catch (e: Exception) {
                                                        ...
                                                        withContext(kotlinx.coroutines.Dispatchers.Main) {
                                                            ...
                                                            android.widget.Toast.makeText(context, context.getString(com.anywaa.connect.R.string.reembedding_memory_done), android.widget.Toast.LENGTH_SHORT).show()
                                                        }
                                                    }
                                                }
                                            } catch (e: Exception) {
                    ...
                    Row(
                        ...
                        Column(
                            ...
                            Text(
                                text = if (FeatureFlags.PREMIUM_ENABLED)
                    ...
                    Row(
                        ...
                        Column(
                            ...
                            Text(
                                text = "Disable background network activity to save data",
                                style = MaterialTheme.typography.bodyMedium,
                        ...
                        Switch(
                            ...
                            onCheckedChange = { enabled ->
                                coroutineScope.launch {
                                    themePreferences.setLowDataModeEnabled(enabled)
                                }
                            }
                        )
                    }
                    ...
                    SettingsItem(
                        ...
                        title = "System Prompt",
                        subtitle = if (systemPrompt.isNullOrBlank()) "Set a persistent instruction for the AI" else "Custom prompt configured",
                        onClick = { showSystemPromptDialog = true }
                    )
                    
                    if (showSystemPromptDialog) {
                        var promptText by remember { mutableStateOf(TextFieldValue(systemPrompt ?: "")) }
                        AlertDialog(
                            onDismissRequest = { showSystemPromptDialog = false },
                            title = { Text("System Prompt") },
                            text = {
                                Column {
                                    Text("This hidden instruction is sent before every conversation to guide the AI's behavior.", style = MaterialTheme.typography.bodyMedium)
                                    Spacer(modifier = Modifier.height(16.dp))
                                    OutlinedTextField(
                                        value = promptText,
                                        onValueChange = { promptText = it },
                                        modifier = Modifier.fillMaxWidth().height(150.dp),
                                        placeholder = { Text("e.g. Always respond in Dha Anywaa") }
                                    )
                                }
                            },
                            confirmButton = {
                                TextButton(onClick = {
                                    coroutineScope.launch {
                                        themePreferences.setSystemPrompt(promptText.text)
                                    }
                    ...
                    if (memoryEnabled) {
                        ...
                        if (showMemoryDialog) {
                            ...
                            val ragServiceManager = com.anywaa.connect.embedding.RagServiceManager.getInstance(context)
                            val coroutineScope = rememberCoroutineScope()
                            ...
                            if (showPickedFilePreview && pickedFileInfo != null) {
                                com.anywaa.connect.components.FilePreviewDialog(
                                        ...
                                        coroutineScope.launch {
                                            try {
                                                ...
                                                if (content != null) {
                                                    ...
                                                    val doc = com.anywaa.connect.data.MemoryDocument(
                                                        id = id,
                                                        fileName = info.name,
                                                        content = content,
                                                        metadata = "uploaded",
                            ...
                            val memoryFlow = remember { com.anywaa.connect.data.AnywaaConnectDatabase.getDatabase(context).memoryDao().getAllMemory() }
                            ...
                            Dialog(onDismissRequest = { showMemoryDialog = false }) {
                                Surface(
                                    ...
                                    Column(
                                        ...
                                        val processorRunning = remember { com.anywaa.connect.data.MemoryProcessor.processing }
        ...
        Dialog(onDismissRequest = { /* block dismiss while running */ }) {
            Surface(
                ...
                Column(
                    ...
                    Text(text = stringResource(id = com.anywaa.connect.R.string.reembedding_memory_in_progress), style = MaterialTheme.typography.bodyLarge)
...
private fun EmbeddingModelSelector(themeViewModel: ThemeViewModel) {
            ...
            try {
                ...
                withContext(kotlinx.coroutines.Dispatchers.IO) {
                    ...
                    try {
                        ...
                        try {
                            withTimeoutOrNull(5_000) { com.anywaa.connect.data.MemoryProcessor.processing.first { it } }
                        } catch (_: Exception) { }
                        com.anywaa.connect.data.MemoryProcessor.processing.first { running -> !running }
                    } catch (e: Exception) {
    ...
    val downloadedEmbeddingModels = remember(context) {
        com.anywaa.connect.data.ModelData.models
...
fun ChatImportDialog(
    onDismiss: () -> Unit,
    onImport: (List<com.anywaa.connect.data.ChatEntity>) -> Unit
) {
    val context = androidx.compose.ui.platform.LocalContext.current
    val db = remember { com.anywaa.connect.data.AnywaaConnectDatabase.getDatabase(context) }
    val allChats by db.chatDao().getNonEmptyChats().collectAsState(initial = emptyList())
    val selectedChats = remember { mutableStateListOf<com.anywaa.connect.data.ChatEntity>() }
...
fun EditMemoryDialog(
    memory: com.anywaa.connect.data.MemoryDocument,
    onDismiss: () -> Unit,
    onSave: (String) -> Unit
) {
    val context = LocalContext.current
