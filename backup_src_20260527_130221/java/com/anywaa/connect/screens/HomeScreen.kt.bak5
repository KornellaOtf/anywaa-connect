...
fun HomeScreen(
    ...
    FeatureFlags.PREMIUM_ENABLED: Boolean = false
) {
    val tools = listOf(
    ...
    Scaffold(
        ...
        Column(
            ...
            LazyVerticalGrid(
                ...
                items(tools) { item ->
                    FeatureCard(item, FeatureFlags.PREMIUM_ENABLED, onNavigateToFeature, onNavigateToPremium)
                }
            }
        }
    }
...
private fun FeatureCard(
    item: FeatureItem,
    FeatureFlags.PREMIUM_ENABLED: Boolean,
    navigate: (String) -> Unit,
    premiumNav: () -> Unit
) {
    val locked = !FeatureFlags.PREMIUM_ENABLED && item.route in setOf("image_generator", "vibe_coder")
    Card(
        modifier = Modifier
            .fillMaxWidth()
            .height(120.dp)
