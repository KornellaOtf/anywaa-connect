...
import com.anywaa.connect.data.*
...
import com.anywaa.connect.utils.AudioConversionUtils
...
import com.anywaa.connect.embedding.ContextChunk
// // import com.google.mediapipe.tasks.genai.llminference.LlmInference
import androidx.lifecycle.ViewModelProvider
import androidx.activity.ComponentActivity
import com.anywaa.connect.ui.components.TtsService
...
class ChatViewModel(
    ...
    private val ragServiceManager = com.anywaa.connect.embedding.RagServiceManager.getInstance(context)
                        ...
                        try {
                            ...
                            try {
                                withTimeoutOrNull(5_000) {
                                    com.anywaa.connect.data.MemoryProcessor.processing
                                        .first { it }
                                }
                            } catch (_: Exception) { }
    ...
    private suspend fun restorePersistedGlobalMemoryAfterRagInit(reason: String) {
        ...
        try {
            ...
            withContext(Dispatchers.IO) {
                com.anywaa.connect.data.MemoryProcessor(context, db).processPending()
            }
        } catch (e: Exception) {
    ...
    private var currentChatIdInternal: String?
        get() = savedStateHandle.get<String>(KEY_CURRENT_CHAT_ID)
        set(value) {
            savedStateHandle.set(KEY_CURRENT_CHAT_ID, value)
            currentChatId = value
        }
    ...
    fun initializeChat(chatId: String, context: Context, creatorId: String? = null) {
        ...
        viewModelScope.launch {
            ...
            if (chatId == "new") {
                ...
                currentChatIdInternal = newChatId
    ...
    viewModelScope.launch {
            ...
            // Apply language mode wrapper (Anywaa Mode always wins if enabled)
            val themePreferences = com.anywaa.connect.data.ThemePreferences(context)
            val isAnywaaMode = themePreferences.anywaaModeEnabled.first()
            val systemPromptText = themePreferences.systemPrompt.first()
            
            if (isAnywaaMode && messageText.isNotEmpty()) {
                modelPromptContent = "Please respond to the following prompt in Dha Anywaa:\n\n$messageText"
            }
            ...
            if (currentModel != null && currentModel!!.isDownloaded) {
                ...
                var history = buildContextAwareHistory(allMessages)
                
                // Inject system prompt if present and not empty
                if (!systemPromptText.isNullOrBlank()) {
                    history = "system: $systemPromptText\n\n$history"
                }
...
inferenceService.loadModel(currentModel!!, _selectedBackend.value, _selectedNpuDeviceId.value)
                                                    ...
                                                    if (globalChunks.isEmpty()) {
                                                        try {
                                                            val persisted = withContext(Dispatchers.IO) {
                                                                com.anywaa.connect.data.AnywaaConnectDatabase.getDatabase(context).memoryDao().getAllChunks()
                                                            }
                    ...
                    // Track precise generation window based on first and last streamed chunks
                                    ...
                                    if (autoReadoutEnabled || _currentTtsMessageId.value == placeholderId) {
                                        ...
                                        val appLocale = com.anywaa.connect.utils.LocaleHelper.getCurrentLocale(context)
    ...
    fun setAgentToolsEnabled(enabled: Boolean) {
        (inferenceService as? com.anywaa.connect.inference.UnifiedInferenceService)?.setAgentToolsEnabled(enabled)
    }
