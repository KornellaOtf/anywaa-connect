...
class MemoryProcessor(private val context: Context, private val db: AnywaaConnectDatabase) {
    private val TAG = "MemoryProcessor"
    private val ragManager = com.anywaa.connect.embedding.RagServiceManager.getInstance(context)
            ...
            try {
                ...
                for (doc in list.filter { it.status == "PENDING" || it.status == "FAILED" }) {
                    try {
                        ...
                        val mgr = com.anywaa.connect.embedding.RagServiceManager.getInstance(context)
                        ...
                        initJob.join()
                            ...
                            try {
                                ...
                                val embeddingBytes = com.anywaa.connect.data.floatArrayToByteArray(emb)
                                val chunkEntity = com.anywaa.connect.data.MemoryChunkEmbedding(
                                    id = chunkId,
                                    docId = doc.id,
                                    ...
                                    embeddingModel = try { com.anywaa.connect.data.ThemePreferences(context).selectedEmbeddingModel.first() } catch (_: Exception) { null },
