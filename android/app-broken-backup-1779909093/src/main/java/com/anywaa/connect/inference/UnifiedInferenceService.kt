...
//         if (model.modelFormat == "gguf" && (nexaService as? com.anywaa.connect.inference.NexaInferenceService)?.isAvailable() != true) {
...
//         if ((nexaService as? com.anywaa.connect.inference.NexaInferenceService)?.isAvailable() == true) {
...
//             "gguf" -> if ((nexaService as? com.anywaa.connect.inference.NexaInferenceService)?.isAvailable() == true) nexaService.getEffectiveMaxTokens(model) else mediaPipeService.getEffectiveMaxTokens(model)
