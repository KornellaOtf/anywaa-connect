...
import com.anywaa.connect.data.isModelFileValid
...
class ModelDownloader(
    ...
    fun downloadModel(model: LLMModel): Flow<DownloadStatus> = flow {
        ...
        // Check for bundled asset and copy if present
        ...
        try {
            context.assets.open(assetPath).use { inputStream ->
                if (!modelFile.exists() || modelFile.length() == 0L) {
                    ...
                    Log.i(TAG, "Copied bundled asset $assetPath to ${modelFile.absolutePath}")
                }
            }
        } catch (e: Exception) {
    ...
    private fun downloadStableDiffusionModel(model: LLMModel): Flow<DownloadStatus> = flow {
        ...
        coroutineScope {
            val extractJob = async(Dispatchers.IO) {
                com.anywaa.connect.utils.ZipExtractor.extractZip(
                    zipFile = zipFile,
    ...
    suspend fun ensureModelAvailable(model: LLMModel): File {
        val modelsDir = File(context.filesDir, "models")
        if (!modelsDir.exists()) modelsDir.mkdirs()
        val modelFile = File(modelsDir, model.localFileName())

        // 1. Check if model is already available and valid
        if (modelFile.exists() && modelFile.length() > 0) {
            val isValid = try { isModelFileValid(modelFile, model.modelFormat) } catch (e: Exception) { false }
            if (isValid) {
                Log.i(TAG, "ensureModelAvailable: Model already present and valid at ${modelFile.absolutePath}")
                return modelFile
            } else {
                Log.w(TAG, "ensureModelAvailable: Existing model is invalid, deleting.")
                modelFile.delete()
            }
        }

        // 2. Check for APK assets (bundled)
        val assetPath = "models/${model.localFileName()}"
        try {
            context.assets.open(assetPath).use { inputStream ->
                Log.i(TAG, "ensureModelAvailable: Copying from APK assets: $assetPath")
                modelFile.outputStream().use { out -> inputStream.copyTo(out) }
                if (isModelFileValid(modelFile, model.modelFormat)) return modelFile
                else modelFile.delete()
            }
        } catch (e: Exception) {
            Log.d(TAG, "ensureModelAvailable: Not in APK assets.")
        }

        // 3. Check Play Asset Delivery (PAD)
        try {
//             val assetPackManager = com.google.android.play.core.assetpacks.// AssetPackManager removed(context)
            val packLocation = assetPackManager.getPackLocation("models_pack")
            if (packLocation != null) {
                val padFile = File(packLocation.assetsPath(), assetPath)
                if (padFile.exists()) {
                    Log.i(TAG, "ensureModelAvailable: Copying from Play Asset Delivery: ${padFile.absolutePath}")
                    padFile.copyTo(modelFile, overwrite = true)
                    if (isModelFileValid(modelFile, model.modelFormat)) return modelFile
                }
            }
        } catch (e: Exception) {
            Log.d(TAG, "ensureModelAvailable: Play Asset Delivery check failed: ${e.message}")
        }

        // 4. Fallback to download flow
        Log.i(TAG, "ensureModelAvailable: Model not found locally. Must be downloaded.")
        // Instead of downloading entirely here, we throw an exception so the UI can trigger
        // `downloadModel(model)` which returns a Flow<DownloadStatus> for progress tracking.
        throw RuntimeException("Model not bundled. Needs download.")
    }
}
