...
package com.anywaa.connect.billing
...
class BillingManager(private val context: Context) {
    ...
    private val _FeatureFlags.PREMIUM_ENABLED = MutableStateFlow(false)
    val FeatureFlags.PREMIUM_ENABLED: StateFlow<Boolean> = _FeatureFlags.PREMIUM_ENABLED.asStateFlow()
    ...
    init {
        if (FeatureFlags.PREMIUM_ENABLED) {
            ...
            _FeatureFlags.PREMIUM_ENABLED.value = true
            ...
            scope.launch {
                prefs.FeatureFlags.PREMIUM_ENABLED.collect { stored ->
                    _FeatureFlags.PREMIUM_ENABLED.value = stored
                }
            }
            connectAndQuery()
        }
    ...
    suspend fun restorePurchasesFromServer(): Boolean {
        ...
        restorePurchasesInternal()
        if (_FeatureFlags.PREMIUM_ENABLED.value) return true
        ...
        return _FeatureFlags.PREMIUM_ENABLED.value
    }
    ...
    private suspend fun setPremium(premium: Boolean) {
        _FeatureFlags.PREMIUM_ENABLED.value = premium
