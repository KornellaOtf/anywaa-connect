...
import com.anywaa.connect.BuildConfig
...
class ModelDownloadViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun downloadModel(model: LLMModel) {
        ...
        val job = viewModelScope.launch {
            var latestStatus: com.anywaa.connect.data.DownloadStatus? = null
    ...
    fun cancelDownload(model: LLMModel) {
        ...
        try {
            val cancelIntent = android.content.Intent(context, com.anywaa.connect.service.ModelDownloadService::class.java).apply {
                action = com.anywaa.connect.service.ModelDownloadService.ACTION_CANCEL_DOWNLOAD
                putExtra(com.anywaa.connect.service.ModelDownloadService.EXTRA_MODEL_NAME, model.name)
            }
        ...
        if ((externalModel.category == "qnn_npu" || externalModel.category == "mnn_cpu") && externalModel.isExtracting) {
            viewModelScope.launch(Dispatchers.IO) {
                try {
                    ...
                    val extractSuccess = com.anywaa.connect.utils.ZipExtractor.extractZip(
                        zipFile = tempFile,
