package com.anywaa.connect.embedding
...
class RagServiceManager(
     ...
     */
    suspend fun restoreGlobalDocumentsFromChunks(chunks: List<com.anywaa.connect.data.MemoryChunkEmbedding>) {
        if (!isReady()) {
        ...
        for (chunk in chunks) {
            try {
                val embedding = com.anywaa.connect.data.byteArrayToFloatArray(chunk.embedding)
    ...
    suspend fun searchRelevantContext(chatId: String, query: String, maxResults: Int = 3, relaxedLexicalFallback: Boolean = false, queryEmbedding: FloatArray? = null): List<ContextChunk> {
        ...
        return if (service != null) {
            try {
                ...
                try {
                    val memoryEnabled = com.anywaa.connect.data.ThemePreferences(context).memoryEnabled.first()
